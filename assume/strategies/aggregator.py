...
class LocalRetailerCoordinatedStrategy(UnitOperatorStrategy):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # -------------------------------------------------
        self.joint_plans: dict[datetime, JointBidPlan] = {}
        self.wm_orderbooks: dict[datetime, Orderbook] = {}
        self.lem_orderbooks: dict[datetime, Orderbook] = {}
        ...
        if self.buy_bid_price < 0.0 or self.sell_bid_price < 0.0:
            raise ValueError(
                f"bid prices must be non-negative, got "
                f"buy={self.buy_bid_price}, sell={self.sell_bid_price}"
            )
    ...
    def _ensure_joint_plan(
        ...
        # -------------------------------------------------
        if decision_time in self.joint_plans:
            return self.joint_plans[decision_time]
        ...
        # -------------------------------------------------
        self.joint_plans[decision_time] = JointBidPlan(
            decision_time=decision_time,
            forecast_snapshot=forecast_snapshot,
            wm_orderbook=wm_orderbook,
            lem_orderbook=lem_orderbook,
        )
        self.wm_orderbooks[decision_time] = wm_orderbook
        self.lem_orderbooks[decision_time] = lem_orderbook
